"""OpenClawAdapter — wraps OpenClaw coding agent."""
...
class OpenClawAdapter(AgentAdapter):
    ...
    def resolve_command(self, user_args: Optional[List[str]]) -> List[str]:
        command = os.getenv("AGENT_SHIELD_OPENCLAW_COMMAND", "openclaw")
        args = os.getenv("AGENT_SHIELD_OPENCLAW_ARGS", "")
        cmd = [command]
        if args:
    ...
    def is_own_process(self, cmdline: str) -> bool:
        ...
        if executable not in {"node", "npx", "openclaw", "python", "python3"}:
            return False
        for token in self._identity_tokens:
            if token in lowered:
                return True
        return executable in {"openclaw", "node", "npx", "python", "python3"}

    def build_identity_tokens(self, cmd: List[str]) -> Set[str]:
        tokens: Set[str] = set()
        for arg in cmd:
            lowered = arg.strip().lower()
            if "openclaw" in lowered and "/" in lowered:
                tokens.add(lowered)
    ...
    def is_own_api_connection(
        ...
        if "openclaw" in lowered:
            return True, "openclaw_api_host"
        return False, ""

    @property
    def responses_api_hosts(self) -> Set[str]:
        hosts = os.getenv("AGENT_SHIELD_OPENCLAW_API_HOSTS", "")
