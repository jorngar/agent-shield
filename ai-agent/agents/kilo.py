"""KiloAdapter — wraps Kilo (Xiaomi Mimo-based coding agent)."""
...
class KiloAdapter(AgentAdapter):
    ...
    def resolve_command(self, user_args: Optional[List[str]]) -> List[str]:
        command = os.getenv("AGENT_SHIELD_KILO_COMMAND", "kilo")
        args = os.getenv("AGENT_SHIELD_KILO_ARGS", "")
        cmd = [command]
        if args:
    ...
    def is_own_process(self, cmdline: str) -> bool:
        ...
        if "kilo" not in lowered:
            return False
        parts = lowered.split()
        if not parts:
            return False
        executable = parts[0].rstrip("/").split("/")[-1]
        if executable not in {"node", "npx", "kilo"}:
            return False
    ...
    def build_identity_tokens(self, cmd: List[str]) -> Set[str]:
        ...
        for arg in cmd:
            ...
            if "kilo" in lowered and "/" in lowered:
                tokens.add(lowered)
    ...
    def is_own_api_connection(
        ...
        if any(h in lowered for h in ("xiaomi", "mimo", "kilo")):
            return True, "kilo_api_host"
        return False, ""

    @property
    def responses_api_hosts(self) -> Set[str]:
        hosts = os.getenv("AGENT_SHIELD_KILO_API_HOSTS", "")
